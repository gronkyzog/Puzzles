...
def ZDDIntersect(F,G):
	...
	def intersect(fhash,ghash):
		...
		if fhash == FalseHash or ghash == FalseHash:
			...
			hhash0 = intersect(fhash,ghash0)
			val = tuple([v,hhash0,TrueHash])
			head = hash(val)
			hashMap[key] = head
			ZDD[head] = val
			return head
			...
			if v == w:
				hhash0 = intersect(fhash0,ghash0)
				hhash1 = intersect(fhash1,ghash1)
				val = tuple([v,hhash0,hhash1])
				head = hash(val)
				hashMap[key] = head
				ZDD[head] = val
				return head
...
for i,x in enumerate(cycle(ZDD3,0),start=1):
 	print i,x
